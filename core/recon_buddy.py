#!/usr/bin/env python3
"""
ReconBuddy Core Module
This module contains the main ReconBuddy class that orchestrates all reconnaissance operations.
"""
import asyncio
import logging
import os
...
class ReconBuddy:
    ...
    async def run_github_recon(self) -> Dict[str, Any]:
        ...
        findings = await github_recon.run_github_recon(org_name, self.is_company)
...
def main():
    ...
    parser.add_argument("-t", "--target", required=True, help="Target domain, IP, or company name")
    ...
    # Load config and get GitHub token if not provided in args
    config = load_config()
    if not args.github_token and config.get('api_keys', {}).get('github_token'):
        args.github_token = config['api_keys']['github_token']
        logger.debug("Using GitHub token from config file")
    
    try:
        logger.debug("Creating ReconBuddy instance...")
        ...
        logger.debug("ReconBuddy instance created successfully")
        
        logger.debug("Starting ReconBuddy run...")
        asyncio.run(recon_buddy.run())
        logger.debug("ReconBuddy run completed")
    except Exception as e:
